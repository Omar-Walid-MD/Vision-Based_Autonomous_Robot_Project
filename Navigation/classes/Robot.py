...
class Robot(NodePath):
    ...
    def calculate_points(self, target):
        ...
        points = [self.scene.grid_to_sim(p) for p in points]
        points[0] = pos
        return points
    ...
    # ---------- CORRECTED SIMULATE NAVIGATION (sends done status) ----------
    def simulate_navigation(self):
        ...
        # If last waypoint, publish done status
        if self.point_index >= len(self.points) - 1:
            self.node.send("navigation/status", {"status": "done"})
            self.points = []
            self.point_index = 0

    # ---------- NEW SIMULATE BATTERY (point‑in‑polygon, charge only at charger) ----------
    def simulate_battery(self):
        actual_x, actual_y = self.getX(), self.getY()
        at_charger = self._is_at_charger(actual_x, actual_y)

        if at_charger:
            self.status.batteryLevel = min(100, self.status.batteryLevel + globalClock.getDt())
            self.status.recharging = (self.status.batteryLevel < 100)
        else:
            self.status.batteryLevel = max(0, self.status.batteryLevel - globalClock.getDt())
            self.status.recharging = False

    # ---------- Point‑in‑polygon helpers for charger detection ----------
    def _is_at_charger(self, x, y):
        charger_zone = self.scene.zones.get("charger")
        if not charger_zone:
            return False
        verts_grid = charger_zone["vertices_grid"]
        # Convert grid vertices to simulation coordinates
        sim_verts = [self.scene.grid_to_sim([gx, gy]) for gx, gy in verts_grid]
        return self._point_in_polygon(x, y, sim_verts)
    ...
    def _point_in_polygon(self, x, y, poly):
        ...
        for i in range(n):
            x1, y1 = poly[i]
            x2, y2 = poly[(i + 1) % n]
            # Does the edge cross the horizontal line at y?
