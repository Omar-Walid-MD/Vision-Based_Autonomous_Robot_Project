...
#define START_FRAME         0xABCD       // [-] Start frme definition for reliable serial communication
  ...
  template <typename O,typename I> void HoverSend(O& oSerial, uint8_t iSlave, I iSpeed, uint8_t  wState=32)
  {
    ...
    // uint8_t* raw = (uint8_t*)&oData;

    // Serial.print("TX: ");
    // for (int i = 0; i < sizeof(SerialServer2Hover); i++) {
  ...
  template <typename O>
  void HoverSendBytes(O& oSerial, const uint8_t* data, size_t len)
  {
      oSerial.write(data, len);
  }

  void sendHexString(HardwareSerial& serial, const String& input)
  {
      String cmd = input;
      cmd.trim();
      ...
      {
          int dash = cmd.indexOf('-', start);

          String token;
          if (dash == -1) {
              token = cmd.substring(start);
              start = cmd.length();
          } else {
              token = cmd.substring(start, dash);
              start = dash + 1;
          }
          ...
          uint8_t value = (uint8_t) strtoul(token.c_str(), nullptr, 16);
          buffer[count++] = value;
      }
