...
// compilation command:
// g++ apriltag.cpp -o apriltag `pkg-config --cflags --libs opencv4 gstreamer-1.0 gstreamer-app-1.0` -lapriltag -lpthread
...
int main(int argc, char* argv[]) {
    ...
    if (!cap.isOpened()) {
        std::cerr << "Cannot open camera with GStreamer pipeline\n";
        return -1;
    }
    std::cout << "Camera opened successfully via GStreamer\n";
