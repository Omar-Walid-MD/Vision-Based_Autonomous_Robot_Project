...
class IsBatteryLow(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, node):
        ...
        self._was_low = False  # Track state changes
    ...
    def update(self):
        ...
        # If the battery WAS low, but is now HIGH (> Vth), start listening!
        if self._was_low:
            self._node.send("voice/command", {"action": "listen"})
            self._was_low = False # Reset state
...
class MoveToCharger(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if status == "done":
            # ── FUSED CLEANUP LOGIC ──────────────────────────────────────────
            # The moment we succeed, we immediately stop the voice listener here
            self._node.send("voice/command", {"action": "stop_listening"})
            # ─────────────────────────────────────────────────────────────────
...
def create_emergency_tree(node):
    ...
    battery_seq.add_children([IsBatteryLow(node), MoveToCharger(node)])
